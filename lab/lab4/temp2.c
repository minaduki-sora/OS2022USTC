...
int write_fat_entry(FAT16 *fat16_ins, WORD clusterN, WORD data) {
  ...
  for(uint i=0; i<fat16_ins->Bpb.BPB_NumFATS; i++) {
    ...
    sector_read(fd, FirstFatSecNum + i * fat16_ins->Bpb.BPB_FATSz16 + ClusterSec, SectorBuffer);
    memcpy(&SectorBuffer[SecOffset], &data, 2);
    sector_write(fd, FirstFatSecNum + i * fat16_ins->Bpb.BPB_FATSz16 + ClusterSec, SectorBuffer);
    /*** END ***/
  }
  return 0;
}
...
WORD alloc_clusters(FAT16 *fat16_ins, uint32_t n)
{
  ...
  /*** BEGIN ***/
  WORD ClustersN = 2;
  while(ClustersN <= 0xffef && allocated < n)
  {
    if(fat_entry_by_cluster(fat16_ins, ClustersN) == 0x0000)
    {
      clusters[allocated] = ClustersN;
      allocated++;
    }
    ClustersN++;
  }
  ...
  size_t ClusterSize = fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec * sizeof(BYTE);
  //簇缓冲，值为0，用于清零
  BYTE * ClusterBuffer = (BYTE *)malloc(ClusterSize);
  memset(ClusterBuffer, 0, fat16_ins->Bpb.BPB_SecPerClus * fat16_ins->Bpb.BPB_BytsPerSec);
  WORD FirstSecOfCluster; //分配簇的首扇区号
  FILE *fd = fat16_ins->fd;
  for(int i = 0; i < n ; i++)
  {
    write_fat_entry(fat16_ins, clusters[i], clusters[i+1]);
    FirstSecOfCluster = ((clusters[i] - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector;
    //清零分配簇
    fseek(fd, BYTES_PER_SECTOR * FirstSecOfCluster, SEEK_SET);
    fwrite(ClusterBuffer, ClusterSize, 1, fd);
    fflush(fd);
  }
  free(ClusterBuffer);
  /*** END ***/
...
int fat16_mkdir(const char *path, mode_t mode) {
  ...
  printf("--MKDIR: Path Before is %s--\n", path);//debug

  WORD prtClusterN;//父目录首簇号，用于实现..
  ...
  if (findFlag == 1) {
    /*** BEGIN ***/
    WORD NewDirClusterN = alloc_clusters(fat16_ins, 1);
    dir_entry_create(fat16_ins, sectorNum, offset, paths[pathDepth - 1], ATTR_DIRECTORY, NewDirClusterN, 0);
    //读取子目录
    FirstSectorofCluster = ((NewDirClusterN - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector;
    //.
    char dirName[11] = ".          ";
    dir_entry_create(fat16_ins, FirstSectorofCluster, 0, dirName, ATTR_DIRECTORY, NewDirClusterN, 0);
    //..
    dirName[1] = '.';
    dir_entry_create(fat16_ins, FirstSectorofCluster, BYTES_PER_DIR, dirName, ATTR_DIRECTORY, prtClusterN, 0);
    /*** END ***/
  }
  printf("--WARNING: MKDIR failed\n");//debug
  return 0;
}
...
void dir_entry_delete(FAT16 *fat16_ins, off_t offset) {
  ...
  /** TO DO: 删除目录项，或者说，将镜像文件offset处的目录项第一个字节设置为0xe5即可。
...
int fat16_rmdir(const char *path) {
  ...
  printf("--RMDIR: find root success--\n");//debug
  if(res != 0) {
    return -ENOENT; // 路径不存在
  }
  ...
  /** TO DO: 检查目录是否为空，如果目录不为空，直接返回-ENOTEMPTY。
   *        注意空目录也可能有"."和".."两个子目录。
  ...
  /* Calculating the first cluster sector for the given path */
    ...
    printf("--RMDIR: before fsbc--\n");//debug
    ClusterN = Dir.DIR_FstClusLO; //目录项中存储了我们要读取的第一个簇的簇号
    first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
    printf("--RMDIR: after fsbc--\n");//debug
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      {
        if(strcmp("..",path_decode(Dir.DIR_Name)) != 0 && strcmp(".",path_decode(Dir.DIR_Name)) != 0)
        {
          //目录不为空
          return -ENOTEMPTY;
        }
  ...
  /*** BEGIN ***/
  find_root(fat16_ins, &Dir, path, &offset);//找回要删除的目录的目录项dir
  ClusterN = Dir.DIR_FstClusLO;
  printf("--RMDIR: before free cluster--\n");//debug
  ...
  //       你也可以使用你在unlink使用的方法。
  /*** BEGIN ***/
  printf("--RMDIR: before delete entry--\n");//debug
