...
long get_cluster_offset(FAT16 *fat16_ins, uint16_t cluster)
{
  ...
  {
    return fat16_ins->DataOffset + fat16_ins->ClusterSize * (cluster - 2);//无0,1号簇
  }
  return -1;
}
                  ...
                  off_t offset, struct fuse_file_info *fi)
{
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      /** TO DO: 这段代码中，从扇区数据（存放在sector_buffer）中正确位置位置读取到相应的目录项，
      ...
      //将扇区缓冲内容(目录结构体)依次写入Root,大小为BYTES_PER_DIR,起始地址为(i - 1) * BYTES_PER_DIR
      ...
      // 找到目录项且目录项未被删除且不为长文件名项
      if ((Root.DIR_Attr == ATTR_ARCHIVE || Root.DIR_Attr == ATTR_DIRECTORY) && Root.DIR_Name[0] != 0xe5)
      {
          char *filename = (char *)path_decode(Root.DIR_Name);
          filler(buffer, filename, NULL, 0);
      }
      /*** END ***/

      // 当前扇区所有条目已经读取完毕，将下一个扇区读入sector_buffer,16 = BYTES_PER_SECTOR / BYTES_PER_DIR
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {
  ...
  else  // 要查询的目录不是根目录
  {
    ...
    int res;
    res = find_root(fat16_ins, &Dir, path, &offset_dir);
    if(res == 1) return -ENOENT; // no such file
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      // TO DO: 读取对应目录项，并用filler填充到buffer
      /*** BEGIN ***/

      //将扇区缓冲内容(目录结构体)依次写入Dir,大小为BYTES_PER_DIR,起始地址为(i - 1) * BYTES_PER_DIR,取模因为i在跨扇区未重置
      memcpy(&Dir, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);

      // 找到目录项且目录项未被删除且不为长文件名项:attr != 0x0f
      if ((Dir.DIR_Attr == ATTR_ARCHIVE || Dir.DIR_Attr == ATTR_DIRECTORY) && Dir.DIR_Name[0] != 0xe5)
      {
          char *filename = (char *)path_decode(Dir.DIR_Name);
          filler(buffer, filename, NULL, 0);
      }
      ...
      if (i % 16 == 0)
      {
        ...
        if (DirSecCnt < fat16_ins->Bpb.BPB_SecPerClus)
        {
          // TO DO: 将下一个扇区的数据读入sector_buffer
          /*** BEGIN ***/
        ...
        else // 当前簇已经读完，需要读取下一个簇的内容
        {
          ...
          // TO DO: 读取下一个簇（即簇号为FatClusEntryVal的簇）的第一个扇区
          ...
          ClusterN = FatClusEntryVal;   //下一个簇号为FAT表项的值
          first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
          i = 0;    //当前扇区目录项计数重置(跨簇)
          DirSecCnt = 1;    //当前簇扇区号重置为1
               ...
               struct fuse_file_info *fi)
{
  ...
  //文件占用的总扇区数(截取至size+offset位置)
  int secnum;
  secnum = (size + offset) / BYTES_PER_SECTOR;
  if(secnum * BYTES_PER_SECTOR < size + offset) secnum++; //向上取整
  //file buffer
  BYTE * file_buffer = (BYTE *)malloc(secnum * BYTES_PER_SECTOR * sizeof(BYTE));
  //sector_buffer
  BYTE sector_buffer[BYTES_PER_SECTOR];
  ...
  int seccnt;//已读取总扇区数
  int sec;  //一簇中当前所读扇区序号
  int SecNumPerClus = fat16_ins->Bpb.BPB_SecPerClus;//一簇所含的扇区数
  BYTE * file_buffer_p = file_buffer;//指向当前所写位置
  //读取文件到file_buffer
  for(seccnt = 0 , sec = 0; seccnt < secnum ; seccnt++)
  {
    //current cluster
    sector_read(fat16_ins->fd, FirstSectorofCluster + sec, file_buffer_p);
    sec++;
    file_buffer_p += BYTES_PER_SECTOR;  //一次读取一个扇区

    //next cluster
    if(sec % SecNumPerClus == 0)
    {
      ClusterN = FatClusEntryVal;
      first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
      sec = 0;//重置所读扇区序号
  ...
  {
    memcpy(buffer , file_buffer + offset , size);
    free(file_buffer);
    return size;
  }
  free(file_buffer);
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    {
      //i为目录项序号,取模后为一个扇区内目录项的序号对应的偏移量
      //将扇区缓冲内容(目录结构体)依次写入Root,大小为BYTES_PER_DIR,起始地址为(i - 1) * BYTES_PER_DIR
      memcpy(&Root, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);

      // 当未曾找到,而当前的目录项为空或已经被删除,则记录sectorNum,offset
      if ( Root.DIR_Name[0] == 0x00 || Root.DIR_Name[0] == 0xe5 )
      {
        if(findFlag == 0)//未找到才记录
        {
          sectorNum = RootDirCnt - 1 + fat16_ins->FirstRootDirSecNum;
          offset = ((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR;  //Root = sector_buffer[offset]
          findFlag = 1;
        }
      }
      else if(strcmp(orgPaths[pathDepth - 1],path_decode(Root.DIR_Name)) == 0)
      {
        //同名文件
        findFlag = 0;
        break;
  ...
  else
  {
    ...
    /* Finds the first corresponding directory entry in the root directory and
     * store the result in the directory entry Dir */
    printf("--MKNOD: before find root\n");//debug
    int res = 1;
    res = find_root(fat16_ins, &Dir, prtPath, &offset_dir);
    if(res == 1){
      printf("--MKNOD: find_root failed");//debug
      return -ENOENT;
    } // no such file
    ...
    for (i = 1; 1 ; i++)
    {
      ...
      if ( Dir.DIR_Name[0] == 0x00 || Dir.DIR_Name[0] == 0xe5 )
      {
        if(findFlag == 0)//未找到才记录
        {
          sectorNum = DirSecCnt - 1 + FirstSectorofCluster;
          offset = ((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR;  //dir = sector_buffer[offset]
          findFlag = 1;
        }
        if(Dir.DIR_Name[0] == 0x00) break;//目录项结束//
      }
      else if(strcmp(orgPaths[pathDepth - 1],path_decode(Dir.DIR_Name)) == 0)
      {
        //同名文件
        findFlag = 0;
        break;
  ...
  if (findFlag == 1) {
    ...
    dir_entry_create(fat16_ins, sectorNum, offset, paths[pathDepth - 1], ATTR_ARCHIVE, 0xff, 0);//未扩容
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  /**
     * TO DO:将创建好的新表项信息写入到磁盘
     **/
  /* Write the above entry to specified location */
  /*** BEGIN ***/
  //sector_write(fat16_ins->fd, sectorNum(offset), entry_info);
  fseek(fat16_ins->fd, fat16_ins->Bpb.BPB_BytsPerSec * sectorNum + offset, SEEK_SET);
  int w_size = fwrite(entry_info, BYTES_PER_DIR, 1, fat16_ins->fd);
  //debug
  if(w_size == NULL) perror("--CREATE ERROR");//debug
  fflush(fat16_ins->fd);
  /*** END ***/
  free(entry_info);
  return 0;
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  printf("--FREE_CLUSTER: before fsbc--\n");//debug
   ...
   **/
  /*** BEGIN ***/
  WORD FATOffset = ClusterNum * 2;  //表项为2字节，因此按字节寻址的偏移为簇号*2
  WORD buffer = 0;    //空闲簇
  printf("--FREE_CLUSTER: before fat1--\n");//debug
  //fat1
  fseek(fd, fat16_ins->FatOffset + FATOffset, SEEK_SET);
  fwrite(&buffer, 2, 1, fd);
  fflush(fd);
  printf("--FREE_CLUSTER: before fat2--\n");//debug
  //fat2
  fseek(fd, fat16_ins->FatOffset + fat16_ins->FatSize + FATOffset, SEEK_SET);
  fwrite(&buffer, 2, 1, fd);
  fflush(fd);
...
int fat16_unlink(const char *path)
{
  ...
  while( ClusterN > 1 && ClusterN < 0xffef )
  {
  ...
  if (strcmp(prtPath, "/") == 0)
  {
    ...
    for (i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      //找到要删除的文件
      if(strcmp(orgPaths[pathDepth - 1],path_decode(Root.DIR_Name)) == 0)
      {
        //同名文件
  ...
  else /* Else if parent directory is sub-directory */
  {
    ...
    DIR_ENTRY Dir;
    off_t offset_dir;
    printf("--UNLINK: before find root\n");//debug
    /* Finds the first corresponding directory entry in the root directory and
     * store the result in the directory entry Dir */
    int res;
    res = find_root(fat16_ins, &Dir, prtPath, &offset_dir);
    ...
    for (i = 1; Dir.DIR_Name[0] != 0x00 ; i++)
    {
      ...
      if(strcmp(orgPaths[pathDepth - 1],path_decode(Dir.DIR_Name)) == 0)
      {
        //同名文件
